  ...
  try {
    ...
    for (const itemId of itemIds) {
      try {
        ...
        console.error(`Failed to backup news item ${itemId}:`, error);
      }
    }
    ...
    for (let i = 0; i < itemIds.length; i++) {
      try {
        ...
        console.error(`Failed to update order for news item ${itemIds[i]}:`, error);
      }
    }
      ...
      try {
        for (const backupItem of backup) {
          try {
            ...
            console.error(`Failed to rollback news item ${backupItem.id}:`, rollbackError);
          }
        }
      } catch (rollbackError) {
    ...
    return NextResponse.json({ error: 'Failed to update news order' }, { status: 500 });
  }
}
